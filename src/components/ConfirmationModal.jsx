...
const ConfirmationModal = ({ isOpen, closeModal, config, onConfirm }) => {
  ...
  const basePrice = isSleeveSelected
    ? config.totalPrice - (config.totalPrice - config.basePrice)
    : config.totalPrice;
  const sleeveCost = isSleeveSelected
    ? config.totalPrice - config.basePrice
    : 0;
        ...
        {config ? (
          <>
            <div className="modal-summary">
              ...
              {isSleeveSelected && (
                <div className="summary-item summary-addon">
                  <span className="summary-label">
                    PolePad Sleeves ({config.poles}x, {config.width},{" "}
                    {config.length},{" "}
                    <span className="capitalize">{config.color}</span>)
                  </span>
                  <span className="summary-value">
                    {formatCurrency(sleeveCost)}
                  </span>
                </div>
              )}
              ...
              {!isSleeveSelected && (
                <div className="summary-item summary-addon">
                  <span className="summary-label">PolePad Sleeves</span>
